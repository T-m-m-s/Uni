...
public class Huffman {
  ...
  //Perché il codice possa funzionare c'è bisogno di un metodo che sfrutti la classe HuffmanStatistics, sia
  //per la compressione che per la decompressione:
  
  //Compressione con HuffmanStatistics 
  public static void compressStat( String src, String dst){
      
      Node root = HuffmanStatistic.frequenze(); //Siccome .frequenze() da solo comprende sia la creazione dell'array delle frequenze 
                                                //che il Nodo per le frequenze, possiamo riassumere : int[] freq = charHistogram( src ); Node root = huffmanTree( freq ); 
                                                //in quest'unico passaggio 
     
      String[] codes = huffmanCodesTable( root ); //tabella (array) dei codici 
      
      InputTextFile in = new InputTextFile(src); //apriamo il lettura il documento originale
      
      OutputTextFile out = new OutputTextFile( dst );  //apriamo in scrittura il documento in cui andremmo a scrivere il file compresso 
      
      while( in.textAvailable()) {    //finchè c'è del testo
          
          // scansione: codifica dei caratteri, carattere per carattere
          char c = in.readChar(); //legge il carattere
          
          out.writeCode( codes[c] ); //ne scrive il relativo codice 
      }
  ...
  //Decompressione con Huffman Statistics
  public static void decompressStat(String src, String dst){
      
      InputTextFile in = new InputTextFile( src ); //apertura del file da comprimere in lettura
      
      Node root = HuffmanStatistic.frequenze(); //creazione del nodo delle frequenze come fatto in compress()
      
      OutputTextFile out = new OutputTextFile( dst );  //apertura del file in cui comprimere in scrittura
      
      while(in.textAvailable()){ //finchè c'è del testo rimasto 
          
          char c = decodeNextChar( root, in ); //decodifica il carattere
          out.writeChar( c ); //e ci scrive la decodifica 
