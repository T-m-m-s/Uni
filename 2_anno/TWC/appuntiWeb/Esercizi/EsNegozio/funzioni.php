...
function registra($nome, $descrizione, $prezzo, $immagine) {
    global $MAGAZZINO;
    ...
    $prod = $ultimo ."#".date("Y-m-d, G:i") . "#". $nome . "#" . $descrizione . "#" . $prezzo . "#" . $immagine. "\n";
    fwrite($fp, $prod);
...
function leggi($da, $quanti = NULL) {
    ...
    for ($i = $da; ($i - $da < $quanti) && ($i <= count($contenuto)); $i++) { // estraggo un prodotto dal file e lo aggiungo all'array $risultato
...
function utenteValido($utente, $password) {
    ...
    return ($utente == $AZIENDA && $password == $PASSWORD);
}
?>