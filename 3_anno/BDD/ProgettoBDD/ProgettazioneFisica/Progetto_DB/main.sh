...
if [ "$DB_HOST" = "localhost" ] || [ "$DB_HOST" = "127.0.0.1" ]; then
    if command -v systemctl >/dev/null 2>&1; then
        info "Verifica Servizio PostgreSQL"
        if systemctl is-active --quiet postgresql; then
            pass "Il servizio PostgreSQL è attivo"
        else
            step "Il servizio PostgreSQL non è attivo. Attivazione in corso..."
            if sudo systemctl start postgresql; then
                pass "Servizio PostgreSQL avviato correttamente"
            else
                fail "Impossibile avviare il servizio PostgreSQL con 'sudo systemctl start postgresql'"
                exit 1
            fi
        fi
        echo ""
...
echo -e "${BOLD}==================================================${NC}"
pass "Esecuzione completata con successo!"
echo -e "${BOLD}==================================================${NC}"
